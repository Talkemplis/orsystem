...
# אפליקציית Streamlit
st.title("מערכת הקצאת שומרים")

data = load_data()
history = load_history()

# הצגת נתונים לדיוק
st.write("נתוני שומרים:", data)
st.write("היסטוריה:", history)

# הוספת שומרים
st.header("הוספת שומר")
guard_name = st.text_input("שם השומר:")
guard_team = st.text_input("צוות השומר:")
if st.button("הוסף שומר"):
    if guard_name and guard_team:
        ...
        st.success(f"שומר {guard_name} נוסף בהצלחה!")
    else:
        st.warning("נא להזין שם וצוות.")

# מחיקת שומרים
st.header("מחיקת שומר")
if "שם" in data.columns:
    guard_to_remove = st.selectbox("בחר שומר למחיקה:", data["שם"].tolist())
    if st.button("מחק שומר"):
        data = data[data["שם"] != guard_to_remove]
        data.to_csv(CSV_FILE, index=False, encoding='utf-8-sig')
        st.success(f"שומר {guard_to_remove} נמחק בהצלחה!")
else:
    st.warning("העמודה 'שם' לא קיימת בנתונים.")

# הצגת רשימת שומרים
st.header("רשימת שומרים")
st.write(data)
